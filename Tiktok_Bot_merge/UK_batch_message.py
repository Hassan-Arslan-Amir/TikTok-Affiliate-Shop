    ...
    try:
        if productstatus == 1:
            ...
            try:
                ...
                productbtn.click()
                time.sleep(2)
                print("Clicked on Sidebar (Retry)")
            ...
            try:
                ...
                for _ in range(3):
                    print("Retrying to find and send Product Card")
                    Card = WebDriverWait(driver, 5).until(
                        EC.presence_of_element_located(
                            (By.XPATH, "//div[@data-e2e='ee7482a9-a8f4-7400']")
                        )
        ...
        print(f"Error in product processing: {e}")
    ...
    try:
        for _ in range(2):
            try:
                ...
                print(f"Error with message area1: {e}")
...
def handle_popup_only_when_occur(driver):
    try:
        ...
        if close_icons:
            print("Popup detected. Invoking handler...")
            handleHomepopup(driver)
        else:
            print("No popup detected.")
    except Exception as e:
        print(f"Error while checking for or handling popup: {e}")
        ...
        try:
            try:
                ...
                if current_url.startswith(expected_url):
                    print("Successfully logged in and on correct URL.")
                else:
                    print("Unexpected URL after login:", current_url)
            except Exception as e:
                print("Login or cookie injection failed:", e)
            ...
            print(f"First iteration started at {start_time}")
            ...
            if not saved_users_from_founduser_file:
                    print("No new users to message after filtering.")
                    driver.quit()
                    return
            ...
            for user in saved_users_from_founduser_file:
                #   count=count+1
                if count == 20: # users_count users number to be fetched from the database for process
                    driver.get(
                        "https://affiliate.tiktok.com/connection/creator-management?view_blocked_creator=&shop_region=GB"
                    )
                    try:
                        handle_popup_only_when_occur(driver)
                    except:
                        print("Error handling creator management popup")
                    #               message= '''
                    ...
                    count = 0
                    print("Removing creators from list")
                    #driver.quit()
                    #time.sleep(1)
                    for i in range(20):
                        try:
                            ...
                            print("Creator removed (or already gone)")
                ...
                try:
                    try:
                        ...
                        if check == 1:
                            ...
                            print(f"{user} marked as processed and deleted from pending list.")
                        else:
                            print("Couldn't click on user")
                    except Exception as e:
                        print("Error messaging user:", user, e)
            ...
            print("Bot crashed due to error:", e)
            driver.quit()
...
def remove_creators(driver):
    try:
        ...
        try:
            ...
            print(f"Failed to click 3-dot menu: {e}")
            return
        ...
        try:
            ...
            batch_button2.click()
            print("Clicked 'Remove' from dropdown")
            time.sleep(1)
        except Exception as e:
            print(f"Failed to click 'Remove' from dropdown: {e}")
            return
        ...
        try:
            ...
            print(f"Failed to confirm removal: {e}")
            return

        time.sleep(1)
        print("Creators removed successfully.")

    except Exception as e:
        print(f"General exception in remove_creators: {e}")
...
#------------------------ Fetch product data from DB
        ...
        if not products:
            print(f"No products found for shop ID: {shop_id}")
            return
# -------------------------Step 1: Click "Batch message" button
        try:
            ...
            print("Batch message button not clicked on attempt:")

        print("Moving to checkbox")
        time.sleep(5)
...
#------------------------- Step 3: Click "Batch message" confirm button
        ...
        print("Clicking batch message button...")
...
#------------------------ Step 4: Click "Add product" button
        for productid, _ in products:  # Only using productid for now
            print(f"Processing product ID: {productid}")
            ...
            print("Add product dialog found")
            time.sleep(1)
...
#------------------------- Step 5: Select "Product ID" from dropdown
            ...
            print("Product name dropdown clicked")
            ...
            print(f"Searching for product ID: {productid}")
#---------------------- Step 6: Input product ID and submit
            ...
            print(f"Entered and submitted product ID: {productid}")
#--------------------- Step 7: Select the matching product checkbox
            ...
            time.sleep(1) 
            driver.execute_script("arguments[0].click();", checkbox)
            print("Product checkbox found and clicked.")
#--------------------- Step 8: Clicking the add button
            ...
            print(f"Error in sendbatch (product section): {e}")
#------------------------ Handling the second input for adding a message
    ...
    try:
        ...
        if dropdown_btn_66:
            ...
            print("Neither dropdown button found.")
    except Exception as e:
        print(f"Error entering message body: {e}")
#------------------------- Handling the final send message button
    try:
        ...
        print("Error occurred while clicking Send message button")
    try:
        time.sleep(1)
...
def automationStepsforfindusernames(driver, name):
    ...
    start = time()
    searchBox2 = '//input[@placeholder="Search names, products, hashtags or keywords"]'
    ...
    try:
        ...
        searchUser = WebDriverWait(driver, 10).until(
            EC.presence_of_element_located((By.XPATH, searchBox2))
        )
        print("Search input box found!")
        driver.execute_script(
            "arguments[0].scrollIntoView({behavior: 'smooth', block: 'center'});",
            searchUser,
        )
        print("Scrolled to search input.")
        searchUser.click()
        print("Clicked on the search input.")

#-------------------- Check and clear previous input
        input_value = searchUser.get_attribute("value")
        print(f"Current value in search box: '{input_value}'")

        if input_value:
            print("Clearing old input...")
            clear_input_like_human(searchUser)

        userName = name
        print(f"Typing '{userName}' into the search box...")
        ...
        print(f"Waiting for action button for user '{name}'...")
        button = WebDriverWait(driver, 10).until(
            EC.element_to_be_clickable((By.XPATH, userfound))
        )
        print(f"Clicking the button for '{name}'...")
        button.click()
        print(f"Clicked on '{name}' in {round(time() - start, 2)}s")
        return 1
    except Exception as e:
        print("Error during automation:")
        print(f"    - Step: {e.__class__.__name__}")
        print(f"    - Message: {e}")
        print(f"    - Failed while trying to click or type into the search bar or button for '{name}'")
        return 0
